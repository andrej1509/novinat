...
if (!empty($arResult['ITEMS']))
{
	...
	{
		if( !empty($arItem['PROPERTIES']['SPECIALOFFER']['VALUE']) )
			$arItem['R_STICKERS']['SPECIALOFFER']=true;
		if( !empty($arItem['PROPERTIES']['NEWPRODUCT']['VALUE']) )
			$arItem['R_STICKERS']['NEWPRODUCT']=true;
		if( !empty($arItem['PROPERTIES']['SALELEADER']['VALUE']) )
			$arItem['R_STICKERS']['SALELEADER']=true;
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			if ('Y' == $arParams['PRODUCT_DISPLAY_MODE'])
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$cachedFile = CFile::ResizeImageGet($arOffer[PREVIEW_PICTURE][ID], array('width'=>230, 'height'=>310), BX_RESIZE_IMAGE_EXACT, true);
					if ($cachedFile) {
					...
					$cachedFile = CFile::ResizeImageGet($arOffer[PREVIEW_PICTURE_SECOND][ID], array('width'=>230, 'height'=>310), BX_RESIZE_IMAGE_EXACT, true);
					if ($cachedFile) {
						$cachedFile[SRC] = $cachedFile[src];
						unset($cachedFile[src]);
						$arOneRow[PREVIEW_PICTURE_SECOND] = $cachedFile;
					} else $arOneRow[PREVIEW_PICTURE_SECOND] = $arOffer[PREVIEW_PICTURE_SECOND];
					unset($cachedFile);
		...
		$oUrl = $arItem['DETAIL_PAGE_URL'];
		$arItem['DETAIL_PAGE_URL'] = strpos($oUrl, 'http') === 0 ? $oUrl : 'http://'.$_SERVER['SERVER_NAME'].$oUrl;
		$arNewItemsList[$key] = $arItem;
		// var_dump($arItem['DETAIL_PAGE_URL']);die();
	}
