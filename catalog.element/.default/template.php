...
<div class="bx_item_slider" id="<? echo $arItemIDs['BIG_SLIDER_ID']; ?>">
	...
	<span class="bx_bigimages_aligner">
		<img id="<? echo $arItemIDs['PICT']; ?>" src="<? echo $arFirstPhoto['SRC']; ?>" alt="<? echo $strAlt; ?>" title="<? echo $strTitle; ?>">
			<div class="r_stick">
				<?
				if (!empty($arResult['R_STICKERS']['SPECIALOFFER']))
					echo '<div class="r_stick-spec"></div>';
				if (!empty($arResult['R_STICKERS']['NEWPRODUCT']))
					echo '<div class="r_stick-new"></div>';
				if (!empty($arResult['R_STICKERS']['SALELEADER']))
					echo '<div class="r_stick-lider"></div>';
				?>
			</div>
	</span>
<?
...
<?
			foreach ($arOneOffer['SMALL_MORE_PHOTOS'] as &$arOnePhoto)
			{
?>
	<li data-value="<? echo $arOneOffer['ID'].'_'.$arOnePhoto['ID']; ?>" style="width: <? echo $strOneWidth; ?>; padding-top: <? echo $strOneWidth; ?>; height: <? echo $strheight; ?>;"><span class="cnt"><span class="cnt_item" style="background-image:url('<? echo $arOnePhoto['SRC']; ?>');"></span></span></li>
<?
			}
			unset($arOnePhoto);
?>
...
?>
	...
	<div class="<?= $boolDiscountShow ? 'item_current_price_sale': 'item_current_price'?>" id="<? echo $arItemIDs['PRICE']; ?>"><? echo $arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']; ?></div>
...
?>
			<div class="item_info_section">
	<!-- <a href="javascript:void(0)"  class="fav-btn <? if (in_array($arResult["ID"],$arBasketItems )) echo ' favactive '; ?>"
		onclick="add2delay(
		'<?=$arResult["ID"]?>',
		...
		this)">Отложить</a> -->
				...
				<a class="wear-table" href="/catalog/tablitsa-razmerov.php">Таблица размеров</a>
			</div>
			<div class="clb"></div>
			<div class="item_info_section">
			<?
