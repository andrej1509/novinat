	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$arWaterMark = Array(
			Array(
				...
				'color' => '#DD1B22',
				'alpha_level' => 70,
				// 'font' => $_SERVER["DOCUMENT_ROOT"].'/bitrix/fonts/pt_sans-bold.ttf',
				'position' => 'bl',
				'file'=>$_SERVER['DOCUMENT_ROOT'].SITE_TEMPLATE_PATH."/watermark.png"
			)
		);
		// var_dump($arWaterMark);
		
		$r_slider_ar = Array();
		foreach ($offerSlider as &$sl_photo) {
			$cachedFile = CFile::ResizeImageGet(
				$sl_photo[ID],
				array('width'=>75, 'height'=>100),
			...
			if ($cachedFile) {
				...
				$cachedFile[ID] = $sl_photo[ID];
				unset($cachedFile[src]);
				$r_slider_ar[] = $cachedFile;
			} else $r_slider_ar[] = $sl_photo;
			unset($cachedFile);

			$cachedFile = CFile::ResizeImageGet(
				$sl_photo[ID],
				array('width' => $sl_photo['WIDTH'], 'height' => $sl_photo['HEIGHT']),
				BX_RESIZE_IMAGE_EXACT,
				true,
				$arWaterMark
			);
			...
			$cachedFile[SRC] = $cachedFile[src];
			$cachedFile[WIDTH] = $cachedFile[width];
			$cachedFile[HEIGHT] = $cachedFile[height];
			$cachedFile[ID] = $sl_photo[ID];
			$sl_photo = $cachedFile;
			unset($cachedFile);

		}
		$arOffer[SMALL_MORE_PHOTOS] = $r_slider_ar;
		unset($r_slider_ar);

		$arOffer['MORE_PHOTO'] = $offerSlider;
		$arOffer['MORE_PHOTO_COUNT'] = count($offerSlider);

		// var_dump($arOffer['MORE_PHOTO']);
		// var_dump($arOffer[SMALL_MORE_PHOTOS]);
...
if( !empty($arResult['PROPERTIES']['SPECIALOFFER']['VALUE']) )
	$arResult['R_STICKERS']['SPECIALOFFER']=true;
if( !empty($arResult['PROPERTIES']['NEWPRODUCT']['VALUE']) )
	$arResult['R_STICKERS']['NEWPRODUCT']=true;
if( !empty($arResult['PROPERTIES']['SALELEADER']['VALUE']) )
	$arResult['R_STICKERS']['SALELEADER']=true;
